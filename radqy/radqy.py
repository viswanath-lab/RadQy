...
#     # Ensure local_variance has a minimum value to avoid division by zero
#     local_variance[local_variance < 1e-6] = 1e-6  # Regularize small variances
...
#     except Exception:
#         # If LBP fails, fallback to a default measure
#         return name, 0.0

#     # Select high-variance texture regions
#     percentile_threshold = 95
#     texture_regions = LBP_texture[(LBP_texture > np.percentile(LBP_texture, percentile_threshold))]
    
#     # If no regions are found, gradually lower the threshold
#     while len(texture_regions) == 0 and percentile_threshold > 50:
#         percentile_threshold -= 5
#         texture_regions = LBP_texture[(LBP_texture > np.percentile(LBP_texture, percentile_threshold))]

#     # If still no texture regions, provide a fallback value
#     if len(texture_regions) == 0:
#         noise_estimate = 1e-6  # Minimum noise value
#     else:
#         noise_estimate = max(np.std(texture_regions), 1e-6)  # Ensure a minimum noise value

#     # Compute signal estimate
#     signal_estimate = np.mean(f) if len(f) > 0 else 1e-6  # Use a small fallback value if f is empty

#     # Calculate SNR with robust handling
#     measure = signal_estimate / noise_estimate
